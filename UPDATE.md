...
| Atualização falha com erro de socket após abrir a nova versão | o updater fazia probes TCP/UDP nas portas de produção, fechava-os e iniciava imediatamente o novo processo | não sondar as portas de produção no updater; aguardar somente 1 s após o encerramento e deixar o `NetworkService` da nova instância fazer o bind real com retry |
| Atualização congela enquanto aguarda a liberação do socket | a nova instância fazia o retry de bind na thread da interface e o updater desistia após 30 s | iniciar o bind pós-update em background, manter retry estendido somente nesse caminho e aguardar até 75 s pela confirmação; a janela permanece responsiva |
| Nova instância falha mesmo sem probes do updater | versões antigas podiam iniciar o helper enquanto o TailMsg ainda mantinha sockets abertos, e o helper podia receber handles no handoff | versões novas param o `NetworkService` antes de criar o helper; o `TailMsgUpdater.exe` também se relança via `ShellExecute` antes de aguardar o processo antigo, cobrindo clientes legados; em caso de falha, reiniciar o serviço na instância antiga |
